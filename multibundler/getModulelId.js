...
function getModuleIdByIndex(projectRootPath,path,entry,isBuz){
  ...
  if(isBuz){
    let entryFileName = entry;
    const entryPathSepLastIndex = entry.lastIndexOf(pathSep);
    if (entryPathSepLastIndex >= 0) {
      entryFileName = entry.substr(entryPathSepLastIndex + 1);
    }
    // console.log('entry:' + entry + ' entryFile:' + entryFileName);
    if(buzMappingPath==null) {
      buzMappingPath = __dirname + pathSep + entryFileName.replace(".js", "") + "Map.json";
    }
    if(buzModuleIdMap.length==0){
      if(fs.existsSync(buzMappingPath)){
        ...
        curBuzModuleId = moduleIdConfig[entryFileName]-1;//根据业务包moduleid的配置取初始modueId
      }
    }
  }
